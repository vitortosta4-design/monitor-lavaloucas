...
def _tentar_buscar_preco_parcelado() -> float:
    """Faz UMA tentativa de abrir a página e extrair o preço parcelado."""
    with sync_playwright() as p:
        ...
        navegador.close()
    # A loja mostra frases assim quando o produto está fora de estoque,
    # em vez de mostrar preço. Nesse caso não faz sentido insistir tentando
    # de novo - é um estado real da loja, não uma instabilidade passageira.
    ...
    if any(marcador in texto_lower for marcador in marcadores_fora_de_estoque):
        raise ProdutoIndisponivelError(
            "O produto está marcado como fora de estoque na WebContinental agora."
        )
...
def buscar_preco_parcelado(tentativas: int = 3, espera_segundos: int = 20) -> float:
    """
    ...
    de desistir - em vez de marcar a execução inteira como falha por causa
    de uma instabilidade passageira. Já se o produto estiver genuinamente
    fora de estoque, não insiste tentando de novo - avisa uma vez só.
    """
    ultimo_erro: Exception | None = None

    for tentativa in range(1, tentativas + 1):
        try:
            return _tentar_buscar_preco_parcelado()
        except ProdutoIndisponivelError:
            # Não é uma instabilidade passageira - não vale a pena tentar de novo.
            raise
        except Exception as erro:
            ultimo_erro = erro
            print(f"Tentativa {tentativa}/{tentativas} falhou: {erro}")
            if tentativa < tentativas:
                print(f"Esperando {espera_segundos}s antes de tentar de novo...")
                time.sleep(espera_segundos)

    # Todas as tentativas falharam - agora sim deixa o erro estourar,
    # pra você ser avisado (via e-mail de falha do próprio GitHub) que
    # algo realmente precisa de atenção.
    raise ultimo_erro
